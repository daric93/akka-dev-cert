...
public class FlightEndpointIntegrationTest extends TestKitSupport {
  ...
  void markSlotAvailableViaHttp() {
    ...
    var availabilityRequest =
        new FlightEndpoint.AvailabilityRequest(
            studentParticipant.id(), studentParticipant.participantType().name());
    ...
    var getResponse =
        httpClient.GET("/flight/availability/" + slotId).responseBodyAs(Timeslot.class).invoke();
  ...
  void markSlotUnavailableViaHttp() {
    ...
    // mark slot available
    httpClient.POST("/flight/availability/" + slotId).withRequestBody(availabilityRequest).invoke();
  ...
  void bookSlotViaHttp() {
    ...
    var availabilityRequestForStudent =
        new FlightEndpoint.AvailabilityRequest(
            studentParticipant.id(), studentParticipant.participantType().name());
    var availabilityRequestForInstructor =
        new FlightEndpoint.AvailabilityRequest(
            instructorParticipant.id(), instructorParticipant.participantType().name());
    var availabilityRequestForAircraft =
        new FlightEndpoint.AvailabilityRequest(
            aircraftParticipant.id(), aircraftParticipant.participantType().name());
    ...
    Assertions.assertEquals(StatusCodes.OK, getResponse.status());
    assertThat(getResponse.body().available())
        .containsExactlyInAnyOrderElementsOf(
            Set.of(studentParticipant, instructorParticipant, aircraftParticipant));
    ...
    var postResponse =
        httpClient.POST("/flight/bookings/" + slotId).withRequestBody(bookingRequest).invoke();
  ...
  void cancelBookingViaHttp() {
    ...
    // cancel booking
    httpClient.DELETE("/flight/bookings/" + slotId + "/" + bookingId).invoke();
    Assertions.assertEquals(StatusCodes.OK, getResponse.status());
    ...
    Awaitility.await()
        ...
        .untilAsserted(
            () -> {
              ...
              Assertions.assertEquals(StatusCodes.OK, getResponse.status());
              assertThat(getResponse.body().slots())
                  .containsExactlyInAnyOrderElementsOf(
                      List.of(
                          new ParticipantSlotsView.SlotRow(
                              slotId_1,
                              studentParticipant.id(),
                              studentParticipant.participantType().name(),
                              "",
                              "available"),
                          new ParticipantSlotsView.SlotRow(
                              slotId_2,
